...
def main():
    ...
    # exclude prs with chatgpt in title
    # prs = prs[~prs['PR Title'].str.contains("chatgpt|openai", case=False, regex=True)]
    # print(f"Total number of PRs without chatgpt in title: {len(prs)}")
    # logger.info(f"Total number of PRs without chatgpt in title: {len(prs)}")
